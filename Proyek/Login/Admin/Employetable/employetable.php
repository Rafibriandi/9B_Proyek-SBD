...
$postgre = "SELECT * FROM employee ORDER BY employee_id ASC";
$result = pg_query($koneksi, $postgre);
?>
 
<html>
<head>    
    <title>Selamat Datang di Tabel Employee</title>
</head>
...
<a href="add.php">Tambah Baris Baru</a><br/><br/>
    ...
    <tr>
        <th>employee_id</th> <th>employee_nama</th> <th>address</th> <th>phone_number</th> <th>email</th><th>Hapus</th>
    </tr>
    <?php  
    while($user_data = pg_fetch_array($result)) {         
        echo "<tr>";
        echo "<td>".$user_data['employee_id']."</td>";
        echo "<td>".$user_data['employee_name']."</td>";
        echo "<td>".$user_data['address']."</td>";
        echo "<td>".$user_data['phone_num']."</td>";      
        echo "<td>".$user_data['email']."</td>";      
        echo "<td><a href='delete.php?id=$user_data[employee_id]'>Delete</a></td></tr>";        
    }
    ?>
    </table>
</body>
